...
class CapillaryCalibration:
    ...
    def sampled_volume_nl(self) -> float:
        ...
        return q * self.observed_seconds * 1e-6
...
GAP_COMPOSITION_UNCERTAINTY = 0.30
NEUTROPHIL_FRACTION_UNCERTAINTY = 0.35
NEUTROPHIL_FRACTION_UNCERTAINTY_ANCHORED = 0.12


def mechanistic_relative_sigma(anchored_to_patient_cbc: bool = False) -> float:
    """Incertidumbre relativa que no proviene del conteo.

    Se combinan los dos terminos en cuadratura, por ser independientes.
    """
    frac = (NEUTROPHIL_FRACTION_UNCERTAINTY_ANCHORED if anchored_to_patient_cbc
            else NEUTROPHIL_FRACTION_UNCERTAINTY)
    return float(np.sqrt(GAP_COMPOSITION_UNCERTAINTY**2 + frac**2))


def combined_relative_ci(n_events: int, confidence: float = 0.95,
                         anchored_to_patient_cbc: bool = False
                         ) -> tuple[float, float]:
    """Intervalo relativo que suma ruido de conteo **y** incertidumbre del metodo.

    Devuelve multiplicadores ``(lo, hi)`` sobre la estimacion puntual.

    Es deliberadamente mas ancho que el de Poisson solo. Un intervalo estrecho
    aqui seria una mentira comoda: sugeriria al clinico una precision que el
    metodo no tiene, y en tamizaje eso se paga en decisiones equivocadas.
    """
    lo_poisson, hi_poisson = poisson_relative_ci(n_events, confidence)
    if not np.isfinite(hi_poisson):
        return lo_poisson, float("inf")

    z = 1.959963985 if confidence >= 0.95 else 1.6448536
    sigma_mech = mechanistic_relative_sigma(anchored_to_patient_cbc)
    sigma_count_lo = abs(np.log(max(lo_poisson, 1e-9))) / z
    sigma_count_hi = abs(np.log(max(hi_poisson, 1e-9))) / z

    lo = float(np.exp(-z * np.sqrt(sigma_count_lo**2 + sigma_mech**2)))
    hi = float(np.exp(+z * np.sqrt(sigma_count_hi**2 + sigma_mech**2)))
    return lo, hi
