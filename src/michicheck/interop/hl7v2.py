...
def _abnormal_flag(decision: TriageDecision) -> str:
    """OBX-8. ``LL`` = criticamente bajo, ``L`` = bajo, ``N`` = normal.

    La bandera describe el valor, no la conducta: el estado del paciente puede
    ser ESTABLE con un recuento en zona de vigilancia, y quien lee el mensaje en
    Galenus necesita ver esa diferencia.
    """
    if decision.level in (RiskLevel.PRIORIZABLE, RiskLevel.GRAVE):
        return "LL"
    if decision.level is RiskLevel.INDETERMINADO:
        return "N"
    return "L" if decision.anc_used < 1500 else "N"


def build_oru_r01(result: ScreeningResult, decision: TriageDecision,
                  patient_id: str, patient_name: str = "",
                  facility: str = "POSTA", sending_app: str = "MICHINAN",
    ...
    obx("ST", f"{METHOD_CODE}-TRI", "Nivel de triaje",
        decision.level.value.upper(), "", _abnormal_flag(decision), "F")
