...
#: Orden canonico de las variables. Se persiste con el modelo: un desajuste de
#: orden entre entrenamiento e inferencia es un error silencioso y grave.
...
#: Variables del modelo compacto (ver justificacion en el docstring del modulo).
COMPACT_FEATURES: tuple[str, ...] = (
    "log_anc_fisico",        # la estimacion fisica: el grueso de la senal
    "edad_anos",             # modula la conversion WBC -> ANC
    "log_eventos_totales",   # cuanta evidencia hay detras del numero
    "log_volumen_nl",        # cuanta sangre se llego a interrogar
)


class YawarClassifier:
    ...
    def fit(self, features: np.ndarray, labels: np.ndarray,
            anc_true: np.ndarray | None = None) -> "YawarClassifier":
        self.pipeline = self._build()
        self.pipeline.fit(self._select(features), labels)

        # Correccion del ANC fisico: regresion lineal en escala log, que es
        # donde el sesgo del detector es (aproximadamente) multiplicativo.
    ...
    @classmethod
    def load(cls, path: str | Path) -> "YawarClassifier":
        import pickle
...
def cross_validate(features: np.ndarray, labels: np.ndarray,
    ...
    for train_idx, test_idx in skf.split(features, labels):
        clf = YawarClassifier(target_sensitivity, random_state, kind, compact)
        clf.fit(features[train_idx], labels[train_idx], anc_true[train_idx])
        proba[test_idx] = clf.predict_proba(features[test_idx])

    tmp = YawarClassifier(target_sensitivity, random_state, kind, compact)
