...
:data:`yawar.optics.MIN_CAPILLARIES_FOR_DECISION` capilares validos y pide mas
captura en su lugar.
"""
...
#: Umbral de confianza de la velocimetria (R^2 validado cruzadamente sobre la
#: banda de 20-200 um). Los valores son bajos en terminos absolutos porque esa
#: banda es una fraccion pequena de la varianza total del kymograph; lo que
#: importa es que ordena bien. Calibrado sobre la cohorte sintetica: a 60 fps
#: un capilar util da 0.04-0.10 y uno inservible cae por debajo de 0.02.
MIN_VELOCITY_CONFIDENCE = 0.03
#: Movimiento **residual** tras estabilizar, en pixeles. El criterio no es
#: "que no se mueva" sino "que lo que quede sea pequeno frente al capilar":
#: el kymograph promedia la seccion transversal (~10 um de lumen util, unos
#: 9 px), asi que un residuo de hasta ~5 px se diluye en ese promedio. Por
#: encima, el muestreo empieza a salirse del lumen y contamina con tejido.
MAX_RESIDUAL_MOTION_PX = 5.0
#: Calidad minima del ajuste del perfil de absorcion para fiarse del diametro.
MIN_DIAMETER_FIT_R2 = 0.80
PLAUSIBLE_DIAMETER_UM = (6.0, 30.0)
# El flujo capilar ungueal real baja de 150 um/s con frecuencia, y aun mas
# en estasis. Un suelo alto recorta la estimacion en silencio.
PLAUSIBLE_VELOCITY_UM_S = (40.0, 2500.0)
MIN_SCANNED_LENGTH_UM = 4000.0
#: Por debajo de esta tasa de captura la velocimetria no es viable: la sangre
#: avanza mas de un periodo del tren eritrocitario entre frames. Medido sobre
#: la cohorte sintetica: 30 fps -> ~81% de error; 60 fps -> ~7%.
...
class ScreeningResult:
    ...
    def probability_severe(self) -> float:
        ...
        calibrado de :mod:`yawar.model` la sustituye cuando existe.
        """
        if not np.isfinite(self.anc_estimate) or self.anc_estimate <= 0:
            return float("nan")
        if self.total_events <= 0:
            return 1.0
        # El conteo es Poisson; en escala log su error relativo es ~1/sqrt(n).
...
def analyze_clip(video: np.ndarray, um_per_px: float, fps: float,
    ...
    # El diametro sale del ajuste del perfil de absorcion, no del umbral:
    # umbralizar lo subestima ~15%, que son ~25% de error en el recuento.
    ...
    # Cuidado con el sesgo mas peligroso de todo el sistema: la confianza de la
    # velocimetria depende de cuantos gaps hay que seguir, es decir que baja
    # justo cuando el nino esta mas neutropenico. Rechazar por baja confianza
    # equivaldria a declarar "no evaluable" precisamente a los pacientes que el
    # tamizaje existe para encontrar, y el sesgo seria invisible en las
    # metricas agregadas. Por eso la baja confianza **no descarta** la medicion:
    # activa la velocidad basal del propio paciente y queda registrada como
...
def aggregate(measurements: list[CapillaryMeasurement], age_years: float,
    ...
    lo_mult, hi_mult = optics.poisson_relative_ci(total_events)
    ...
    if len(usable) < min_capillaries:
        ...
        conclusive = False
    if total_events == 0:
        # Cero eventos no prueba ANC=0: prueba que el limite superior es bajo.
        # Es informativo, pero hay que decirlo asi.
