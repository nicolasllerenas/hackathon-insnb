...
class CapillarySegmentation:
    ...
    centerline_px: np.ndarray   # (N, 2) float, orden x,y a lo largo del vaso
    arclength_um: np.ndarray    # (N,) coordenada de arco
    diameter_um: float
...
#: Alargamiento minimo (longitud / anchura) para aceptar un componente como
#: capilar. Una horquilla ungueal ronda 15-30; una mancha de fondo, 2-5.
MIN_ELONGATION = 6.0
#: Anchura plausible del lumen, en um.
...
def segment_capillaries(video: np.ndarray, um_per_px: float,
    ...
    **Varios umbrales en vez de uno.** No existe un umbral unico valido: en un
    ...
    # No hay un umbral unico que sirva. En un campo real el capilar ocupa un
    # 1-5% de los pixeles y hay que cortar alto; en un ROI recortado alrededor
    # de un solo capilar ocupa el 15-20% y ese mismo corte lo trocea. Como el
    # criterio de aceptacion posterior (forma alargada) es especifico, se puede
    # permitir el lujo de probar varios umbrales y quedarse con lo mejor de
    # todos: un componente con forma de capilar es un capilar, salga del umbral
    # que salga.
    ...
    for umbral in umbrales:
        ...
        for etiqueta in range(1, n):
            ...
            if area < min_area_px:
                continue
            # Evita reanalizar el mismo objeto hallado con otro umbral.
...
def _acumular_candidato(mask: np.ndarray, score: np.ndarray, um_per_px: float,
    ...
    # Se prioriza el capilar con mas senal de flujo y forma mas limpia.
...
def _centerline_and_width(mask: np.ndarray, um_per_px: float
    ...
    # Suavizado del trazo para que la tangente no oscile pixel a pixel.
...
def fit_diameter_um(video: np.ndarray, seg: CapillarySegmentation,
    ...
    # Se muestrea bastante mas ancho que el lumen para incluir tejido a ambos
    # lados: es lo que fija el nivel de referencia I_tejido.
    ...
    # Centrado fino: el minimo del perfil marca el eje del capilar.
...
def _skeletonize(mask: np.ndarray) -> np.ndarray:
    ...
    m8 = mask.astype(np.uint8) * 255
    try:  # pragma: no cover - depende de opencv-contrib
        return cv2.ximgproc.thinning(m8) > 0
    except (AttributeError, cv2.error):
        pass
    # Respaldo: erosiones sucesivas (Lantuejoul).
...
def _order_skeleton_path(pts: np.ndarray) -> np.ndarray:
    ...
    # Grafo de vecindad: se conectan puntos del esqueleto a <=1.5 px (8-vecinos).
    tree = cKDTree(pts)
    ...
    # Doble BFS ponderado: del punto 0 al mas lejano, y de ese al mas lejano.
    # Es el algoritmo estandar para hallar el diametro de un arbol.
    ...
    # Los puntos alcanzables se ordenan por su distancia geodesica al extremo.
