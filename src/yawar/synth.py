...
1. **Contenido del lumen.** Los eritrocitos circulan en fila india. Se modela
   ...
   *huecos* (ocupacion 0) de longitud lognormal (mediana ~30 um), en posiciones
   ...
   :func:`yawar.optics.event_rate_from_wbc`.
...
3. **Absorcion (Beer-Lambert).** El capilar es un cilindro de diametro ``d``.
   ...
   :mod:`yawar.illumination`. Cambiar de 420 nm directo a 530 nm oblicuo altera
...
#: Coeficiente de absorcion efectivo de sangre entera a 420 nm, en um^-1.
#: (banda de Soret de la hemoglobina; ~1000 cm^-1 = 0.1 um^-1)
MU_BLOOD_420NM_PER_UM = 0.1

#: Longitud tipica de un eritrocito visto de canto en fila india (um).
RBC_LENGTH_UM = 8.0
#: Separacion de plasma entre eritrocitos consecutivos (um).
RBC_GAP_UM = 2.0
...
class OpticalSetup:
    ...
    um_per_px: float = 1.0625     # 1360 um / 1280 px del montaje de referencia
    fps: float = 60.0
    duration_s: float = 30.0
    blur_um: float = 2.6          # difraccion + desenfoque residual
    read_noise: float = 2.5       # DN de ruido de lectura
    photon_scale: float = 900.0   # fotones equivalentes a plena escala (ruido shot)
    tremor_um: float = 3.0        # amplitud RMS del temblor con soporte 3D
    tremor_tau_s: float = 0.35    # tiempo de correlacion del temblor
    vignetting: float = 0.25      # caida de iluminacion en los bordes
    illumination_drift: float = 0.04

    # --- iluminacion espectral (ver yawar.illumination) -------------------
    wavelength_nm: float = 530.0  # prototipo KittyScope: LED verde
    oblique: bool = True          # fuente a ~70 grados fuera del eje
    phototype: str = "IV"         # Fitzpatrick; III-V es lo mayoritario en Peru
    oblique_gain: float = 4.0     # ganancia de contraste de la geometria oblicua
    ...
    def mu_effective_per_um(self) -> float:
        ...
        # contraste = 1 - exp(-mu*d)  ->  mu = -ln(1-contraste)/d
...
class CapillaryState:
    ...
    curvature: float = 0.15       # 0 = recto; ~0.3 = arco marcado
    orientation_deg: float = 12.0
    pulsatility: float = 0.25     # amplitud relativa de la modulacion cardiaca
...
class PatientState:
    ...
    def effective_heart_rate_bpm(self) -> float:
        ...
        # Frecuencia cardiaca de reposo aproximada por edad pediatrica.
...
class SyntheticCapture:
    ...
    video: np.ndarray | None           # (T, H, W) uint8
    kymograph: np.ndarray | None       # (T, S) float32 en [0, 1]
    patient: PatientState
    capillary: CapillaryState
    setup: OpticalSetup
    event_material_pos_um: np.ndarray  # posicion de cada leucocito en el patron
    event_gap_len_um: np.ndarray
    displacement_um: np.ndarray        # D(t), una entrada por frame
    pattern_offset_um: float           # patron -> coordenada material
    tremor_px: np.ndarray              # (T, 2) temblor real aplicado
    seed: int

    # -- verdad-terreno derivada ------------------------------------------
...
def _build_lumen_pattern(
    ...
    # Tren de eritrocitos en fila india, con jitter en el espaciado.
    pos = 0.0
    ...
    # Ondas de densidad: el hematocrito capilar no es constante. El efecto
    # Fahraeus, la agregacion eritrocitaria (rouleaux) y el reparto desigual en
    # las bifurcaciones producen tramos mas y menos empaquetados, con longitud
    # de correlacion de ~100 um. En el video se ven como ondas de densidad que
    # viajan con el flujo, y son la estructura de escala larga que hace
    # medible la velocidad cuando casi no hay leucocitos que seguir.
    # ATENCION: la amplitud (25%) es una suposicion a validar contra video real
    # del INSNSB; de ella depende cuanta velocimetria es posible a 60 fps.
    corr_len_um = 110.0
    ...
    # Leucocitos: proceso de Poisson en la coordenada material.
    # cells/um = C[cells/uL] * A[um^2] * 1e-9 [uL/um^3]
    ...
    # Longitud del hueco optico: el leucocito mas la estela de deplecion
    # eritrocitaria aguas abajo. Lognormal con mediana ~30 um.
...
def _displacement(setup: OpticalSetup, cap: CapillaryState,
    ...
    v = np.maximum(v, 0.05 * cap.velocity_um_s)   # el flujo nunca se invierte
    return np.cumsum(v) / setup.fps


# --------------------------------------------------------------------------
# 2. Geometria del capilar
# --------------------------------------------------------------------------
...
def _centerline(cap: CapillaryState, setup: OpticalSetup,
    ...
    # Arco suave: recta + componente cuadratica controlada por la curvatura.
...
def _tissue_background(shape: tuple[int, int], setup: OpticalSetup,
    ...
    for sigma_px, amplitude in (
        (max(h, w) / 12.0, 0.10),   # plexo venoso subpapilar
        (8.0, 0.06),                # dermatoglifos
        (2.0, 0.035),               # granulado superficial
    ):
...
def render_capture(
    ...
    # El contraste y el ruido salen de la configuracion espectral, no de
    # constantes: el prototipo usa 530 nm oblicuo, no 420 nm directo.
    mu_eff = setup.mu_effective_per_um
    photon_scale = setup.effective_photon_scale

    disp = _displacement(setup, capillary, patient)
    # El patron debe cubrir todo el material que llegara a cruzar el campo:
    # desde el que ya esta dentro en t=0 hasta el que entra en el ultimo frame.
    margin_um = 150.0
    ...
    # La escena se renderiza sobre un lienzo mas grande que el ROI y se recorta
    # **despues** de aplicar el temblor. Sin ese margen, el desplazamiento
    # tendria que rellenar los bordes replicando pixeles, y el registro se
    # entrenaria contra un artefacto que en la realidad no existe: el sensor
    # siempre ve escena de verdad al otro lado del encuadre.
    ...
    # Temblor: proceso de Ornstein-Uhlenbeck en x e y.
    ...
    sigma_px = setup.blur_um / setup.um_per_px / 2.355  # FWHM -> sigma
    ...
    # Malla radial para el kymograph analitico (promedio sobre la seccion).
    ...
    for t in range(setup.n_frames):
        # Ocupacion en el marco de laboratorio: el punto de arco s contiene el
        # material que en el patron esta en (s - D(t) + offset).
        ...
        # --- kymograph analitico: promedio de transmitancia en la seccion ---
        ...
        # Ruido de fotones + lectura, en escala 0-255. Se aplica despues del
        # recorte: el ruido lo pone el sensor, no la escena, y por tanto no se
    ...
    # El kymograph tambien lleva ruido, para que el detector no se entrene en
...
class CohortSpec:
    ...
    duration_s: float = 30.0
    # Temblor residual con el dedo apoyado en el soporte impreso. El rango
    # alto (8 um) representa a un nino pequeno inquieto o mal posicionado.
...
def iter_cohort(spec: CohortSpec, with_video: bool = False
    ...
    for i in range(spec.n_samples):
        ...
        # El diametro y la velocidad son propiedades del sujeto; varian poco
        # entre capilares vecinos del mismo dedo.
