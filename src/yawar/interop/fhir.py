...
HL7 v2 (:mod:`yawar.interop.hl7v2`) es lo que hoy entiende un HIS hospitalario
...
SYSTEM_YAWAR = "https://github.com/yawar-nan/codes"

LOINC_ANC = "751-8"
SNOMED_NEUTROPENIA = "165517008"          # Neutropenia
SNOMED_FEBRILE_NEUTROPENIA = "409089005"  # Febrile neutropenia
...
def _interpretation(level: RiskLevel) -> dict[str, Any]:
    code, display = {
        RiskLevel.NEGRO: ("LL", "Critical low"),
        RiskLevel.ROJO: ("LL", "Critical low"),
        RiskLevel.AMARILLO: ("L", "Low"),
        RiskLevel.VERDE: ("N", "Normal"),
        RiskLevel.INDETERMINADO: ("IND", "Indeterminate"),
    }.get(level, ("N", "Normal"))
    return {
        "coding": [{
...
def anc_observation(result: ScreeningResult, decision: TriageDecision,
    ...
    obs: dict[str, Any] = {
        ...
        # Preliminar a proposito: un tamizaje nunca es un resultado final.
        "status": "preliminary",
        "category": [{
            "coding": [{
    ...
    if result.conclusive:
        ...
        obs["interpretation"] = [_interpretation(decision.level)]
        obs["component"] = [
...
def _quantity_component(code: str, display: str, value: float,
    ...
    return {
        "code": {"coding": [{"system": SYSTEM_YAWAR, "code": code,
                             "display": display}]},
...
def quality_observation(result: ScreeningResult, patient_ref: str
    ...
    obs: dict[str, Any] = {
        "resourceType": "Observation",
        "status": "final",
        "code": {"coding": [{"system": SYSTEM_YAWAR, "code": "calidad-captura",
...
def referral_request(decision: TriageDecision, patient_ref: str,
    ...
    urgency = "stat" if decision.level is RiskLevel.NEGRO else "urgent"
    reason_code = (SNOMED_FEBRILE_NEUTROPENIA
                   if decision.level is RiskLevel.NEGRO else SNOMED_NEUTROPENIA)
    return {
...
def build_bundle(result: ScreeningResult, decision: TriageDecision,
    ...
    (:mod:`yawar.interop.pe_core`) y una nota de conformidad que declara
    ...
    report = {
        ...
        "status": "preliminary",
        "code": {"coding": [{"system": SYSTEM_YAWAR,
    ...
    if decision.level is RiskLevel.NEGRO:
        entries.append({"fullUrl": _uuid(),
    ...
    if incluir_pe_core:
        ...
        # La nota de conformidad viaja con el mensaje, no en un documento
        # aparte: dentro de seis meses nadie va a ir a buscar el documento.
